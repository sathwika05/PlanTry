...
public class MeatsList extends AppCompatActivity {
                        ...
                        for (DataSnapshot dataSnapshot : snapshot.getChildren()){
                            if(dataSnapshot.child("email").getValue().equals(profile.getEmail())){
                                ...
                                householdDB.orderByChild("ownerEmail").equalTo(owner).addListenerForSingleValueEvent(new ValueEventListener() {
                                    ...
                                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                                        if(snapshot.exists()){
                                            for(DataSnapshot data : snapshot.getChildren()){
                                                ...
                                                householdDB.child(key).child("meats").addListenerForSingleValueEvent(new ValueEventListener() {
                                                    @Override
                                    ...
                                    @Override
                                    public void onCancelled(@NonNull DatabaseError error) {
                                        Toast.makeText(MeatsList.this, "Add item failed. Please try again", Toast.LENGTH_LONG).show();
                                    }
                                });
                            }
                        }
                                        ...
                                        householdDB.orderByChild("ownerEmail").equalTo(owner).addListenerForSingleValueEvent(new ValueEventListener() {
                                            ...
                                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                                if(snapshot.exists()){
                                                    for(DataSnapshot data : snapshot.getChildren()){
                                                        String key = data.getKey();
                                                        Item meatsItem = new Item(productName, expiration, displayName, isStaples, false);

                                                        if(!isAdded){
                                                            householdDB.child(key).child("meats").push().setValue(meatsItem);
                                                            //    householdDB.child(key).child("meats").child(productName).setValue(meatsItem);
                                                            Toast.makeText(MeatsList.this, productName + " added successfully", Toast.LENGTH_SHORT).show();
