...
public class SeafoodsList extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        categoryHeading.setText("Seafoods");
                        ...
                        for (DataSnapshot dataSnapshot : snapshot.getChildren()){
                            if(dataSnapshot.child("email").getValue().equals(profile.getEmail())){
                                ...
                                householdDB.orderByChild("ownerEmail").equalTo(owner).addListenerForSingleValueEvent(new ValueEventListener() {
                                    ...
                                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                                        if(snapshot.exists()){
                                            for(DataSnapshot data : snapshot.getChildren()){
                                                ...
                                                householdDB.child(key).child("seafoods").addListenerForSingleValueEvent(new ValueEventListener() {
                                                    @Override
                                    ...
                                    @Override
                                    public void onCancelled(@NonNull DatabaseError error) {
                                        Toast.makeText(SeafoodsList.this, "Add item failed. Please try again", Toast.LENGTH_LONG).show();
                                    }
                                });
                            }
                        }
                                        ...
                                        householdDB.orderByChild("ownerEmail").equalTo(owner).addListenerForSingleValueEvent(new ValueEventListener() {
                                            ...
                                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                                if(snapshot.exists()){
                                                    for(DataSnapshot data : snapshot.getChildren()){
                                                        String key = data.getKey();
                                                        Item seafoodsItem = new Item(productName, expiration, displayName, isStaples, false);

                                                        if(!isAdded){
                                                            householdDB.child(key).child("seafoods").push().setValue(seafoodsItem);
                                                            //    householdDB.child(key).child("seafoods").child(productName).setValue(seafoodsItem);
                                                            Toast.makeText(SeafoodsList.this, productName + " added successfully", Toast.LENGTH_SHORT).show();
