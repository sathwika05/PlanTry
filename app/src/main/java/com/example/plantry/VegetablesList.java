...
public class VegetablesList extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        categoryHeading.setText("Vegetables");
                        ...
                        for (DataSnapshot dataSnapshot : snapshot.getChildren()){
                            if(dataSnapshot.child("email").getValue().equals(profile.getEmail())){
                                ...
                                householdDB.orderByChild("ownerEmail").equalTo(owner).addListenerForSingleValueEvent(new ValueEventListener() {
                                    ...
                                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                                        if(snapshot.exists()){
                                            for(DataSnapshot data : snapshot.getChildren()){
                                                ...
                                                householdDB.child(key).child("vegetables").addListenerForSingleValueEvent(new ValueEventListener() {
                                                    @Override
                                    ...
                                    @Override
                                    public void onCancelled(@NonNull DatabaseError error) {
                                        Toast.makeText(VegetablesList.this, "Add item failed. Please try again", Toast.LENGTH_LONG).show();
                                    }
                                });
                            }
                        }
                                        ...
                                        householdDB.orderByChild("ownerEmail").equalTo(owner).addListenerForSingleValueEvent(new ValueEventListener() {
                                            ...
                                            public void onDataChange(@NonNull DataSnapshot snapshot) {
                                                if(snapshot.exists()){
                                                    for(DataSnapshot data : snapshot.getChildren()){
                                                        String key = data.getKey();
                                                        Item vegetablesItem = new Item(productName, expiration, displayName, isStaples, false);

                                                        if(!isAdded){
                                                            householdDB.child(key).child("vegetables").push().setValue(vegetablesItem);
                                                            //    householdDB.child(key).child("vegetables").child(productName).setValue(vegetablesItem);
                                                            Toast.makeText(VegetablesList.this, productName + " added successfully", Toast.LENGTH_SHORT).show();
